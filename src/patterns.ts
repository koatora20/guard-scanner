/**
 * guard-scanner detection patterns.
 */
...
export const PATTERNS: PatternRule[] = [
    ...
    { id: 'PERSIST_CRON', cat: 'persistence', regex: /(?:crontab\b|cron(?:\.d)?\b|at\s+\d{1,2}:\d{2}\b|schtasks(?:\.exe)?\b)/gi, severity: 'HIGH', desc: 'Scheduled task creation', codeOnly: true, owasp: 'LLM06' },
    ...
    { id: 'PII_MY_NUMBER', cat: 'pii-exposure', regex: /(?:マイナンバー|個人番号|My Number)\D{0,12}\d{4}\s*\d{4}\s*\d{4}|\d{4}\s*\d{4}\s*\d{4}\D{0,12}(?:マイナンバー|個人番号|My Number)/gi, severity: 'CRITICAL', desc: 'Potential My Number (個人番号)', all: true, owasp: 'LLM02' },
    ...
    { id: 'AN_MOLTBOOK_PI', cat: 'agent-native-attacks', regex: /(?:other\s+agent|moltbook|swarm)[\s\S]{0,100}(?:ignore|override)|(?:ignore|override)[\s\S]{0,100}(?:other\s+agent|moltbook|swarm)/gi, severity: 'CRITICAL', desc: 'Agent-to-Agent prompt injection (Moltbook vector)', all: true, owasp: 'LLM01' },
    { id: 'AN_WS_HIJACK', cat: 'agent-native-attacks', regex: /new\s+WebSocket\s*\([^)]+\)[\s\S]{0,200}(?:send|emit)\s*\(\s*(?:process\.env|token|credentials|session)/gi, severity: 'CRITICAL', desc: 'WebSocket session exfiltration (CVE-2026-25253)', codeOnly: true, owasp: 'LLM05' },
];
