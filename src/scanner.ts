...
export const VERSION = CAPABILITIES.product.version;

const THRESHOLDS_MAP: Record<string, Thresholds> = {
    normal: { suspicious: 30, malicious: 80 },
    strict: { suspicious: 20, malicious: 60 },
};
export const THRESHOLDS = THRESHOLDS_MAP;
...
const REPO_MARKERS = new Set([
    'package.json', 'Cargo.toml', 'openclaw.plugin.json', '.git', '.jj', 'src', 'dist',
]);
const REPO_SURFACE_DIRS = new Set([
    'src', 'dist', 'scripts', 'hooks', 'bin', 'cli', 'plugin', 'plugins',
]);
const REPO_ROOT_FILES = new Set([
    'package.json', 'Cargo.toml', 'openclaw.plugin.json', 'pyproject.toml',
    'go.mod', 'go.sum', 'tsconfig.json', 'package-lock.json', 'pnpm-lock.yaml', 'yarn.lock',
]);
const REPO_EXCLUDE_DIRS = new Set([
    '.git', '.jj', 'node_modules', 'target', 'coverage', '.next', '.turbo',
    'docs', 'doc', 'output', 'reports', 'report', 'memory', 'archive', '_archive',
    'test', 'tests', '__tests__', 'fixtures', 'examples', 'tmp', 'temp',
]);
...
export class GuardScanner {
    ...
    scanDirectory(dir: string): SkillResult[] {
        ...
        console.log(`📦 Targets found: ${targets.length} (mode: ${this.scanMode})`);
    ...
    private scanTarget(targetPath: string, targetName: string, kind: ScanTargetKind): void {
        this.stats.scanned++;
        ...
        const filtered = findings
            .filter(f => !this.ignoredPatterns.has(f.id))
            .flatMap((finding) => {
    ...
    private enrichFinding(finding: Finding): Finding {
        const fileLower = finding.file.toLowerCase();
        let evidenceClass: Finding['evidence_class'] = 'code';
        let confidence = 0.95;
    ...
    private resolveTargets(dir: string): ScanTarget[] {
        ...
        const immediateDirs = fs.readdirSync(dir, { withFileTypes: true })
            .filter((entry) => entry.isDirectory())
            .map((entry) => path.join(dir, entry.name));

        const directSkills = immediateDirs
            .filter((entry) => this.isSkillDir(entry))
            .map((entry) => ({ path: entry, name: path.basename(entry), kind: 'skill' as const }));
        if (directSkills.length > 0) {
            return directSkills;
        }

        const nestedSkills: ScanTarget[] = [];
    ...
    private safeReadDirs(dir: string): string[] {
        try {
    ...
    private isSkillDir(dir: string): boolean {
        return fs.existsSync(path.join(dir, 'SKILL.md'));
    }

    private looksLikeRepo(dir: string): boolean {
        try {
    ...
    private checkSignatures(content: string, filePath: string, findings: Finding[], kind: ScanTargetKind): void {
            ...
            if (sig.patterns) {
                ...
                for (const pat of sig.patterns) {
                    if (content.includes(pat)) {
                        ...
                        findings.push({
                            severity: sig.severity, id: `SIG_${sig.id}`, cat: 'signature-match',
                            desc: `[${sig.id}] ${sig.name} (${matchedPatterns.length} indicators)`, file: relFile, line: lineNum,
    ...
    private checkComplexity(skillPath: string, skillName: string, findings: Finding[], kind: ScanTargetKind): void {
    ...
    private checkConfigImpact(skillPath: string, skillName: string, findings: Finding[], kind: ScanTargetKind): void {
        const files = this.getFiles(skillPath, kind);
        for (const file of files) {
            const ext = path.extname(file).toLowerCase();
    ...
    private checkCrossFile(skillPath: string, skillName: string, findings: Finding[], kind: ScanTargetKind): void {
        const files = this.getFiles(skillPath, kind);
        ...
        const b64Fragments: Base64Fragment[] = [];
    ...
    private getLineText(content: string | undefined, line?: number): string {
        if (!content || !line || line < 1) return '';
        return content.split('\n')[line - 1] ?? '';
    }

    private getLineWindow(content: string | undefined, line?: number, radius: number = 2): string[] {
        if (!content) return [];
        const lines = content.split('\n');
    ...
    private isBenignPromptContext(windowLower: string): boolean {
        return /#\[cfg\(test\)\]|#\[test\]|mod tests|assert!|assert_eq!/.test(windowLower) ||
            /examples:|usage:|help_text|eprintln!\(|println!\(|demo guard/.test(windowLower);
    }

    private isPatternCatalogContext(lineLower: string, windowLower: string): boolean {
    ...
    private isSchemaFieldContext(lineLower: string, windowLower: string): boolean {
    ...
    private isBenignSecretContext(lineLower: string, windowLower: string, sample?: string): boolean {
        const sampleLower = (sample || '').toLowerCase();
        return /abcdefghijklmnopqrstuvwxyz|decode_table|base64|hex|placeholder|0x123456|assert_eq!|#\[test\]|mod tests/.test(windowLower) ||
            /application\/x-www-form-urlencoded|content-type|authorization|format!\("basic \{\}"/.test(windowLower) ||
            sampleLower.startsWith('abcdefgh') ||
            sampleLower.startsWith('0x123456') ||
            sampleLower.startsWith('applicat');
    }

    private isBenignBase64Fragment(content: string, index: number): boolean {
    ...
    private calculateRisk(findings: Finding[]): number {
        ...
        if (
            findings.some((f) =>
                (f.id === 'IOC_IP' || f.id === 'IOC_URL' || f.id === 'KNOWN_TYPOSQUAT') &&
                !f.fp_suspected,
            )
        ) score = 100;
        ...
        // Compaction persistence amplification
    ...
    scoreFindings(findings: Finding[]): { risk: number; engine: 'ts' | 'rust' } {
    ...
    private getFiles(dir: string, kind: ScanTargetKind, rootDir: string = dir): string[] {
