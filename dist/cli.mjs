...
if (args.includes("--help") || args.includes("-h")) {
  console.log(`
\u{1F6E1}\uFE0F  guard-scanner v${VERSION} \u2014 Agent Skill Security Scanner (TypeScript-first)
...
if (args[0] === "install-check") {
  ...
  const scanner2 = new GuardScanner({ strict: icStrict, verbose: icVerbose });
  const skillName = path.basename(absPath);
  console.log(`
\u{1F6E1}\uFE0F  guard-scanner install-check v${VERSION}`);
...
`);
  ...
  if (icVerbose || findings.length > 0) {
    for (const f of findings) {
      const owaspTag = PATTERNS.find((p) => p.id === f.id)?.owasp || "";
