...
class GuardScanner {
    ...
    scanDirectory(dir) {
        ...
        for (const target of targets) {
            ...
            if (this.selfExclude && targetPath === this.scannerDir) {
                if (!this.summaryOnly)
                    console.log(`⏭️  ${target.name} — SELF (excluded)`);
                continue;
            }
            if (this.ignoredSkills.has(target.name)) {
                if (!this.summaryOnly)
                    console.log(`⏭️  ${target.name} — IGNORED`);
                continue;
            }
    ...
    scanTarget(targetPath, targetName, kind) {
        ...
        if (kind === 'skill' && ioc_db_js_1.KNOWN_MALICIOUS.typosquats.includes(targetName.toLowerCase())) {
            findings.push({
        ...
        if (this.checkDeps)
            this.checkDependencies(targetPath, targetName, findings);
    ...
    resolveTargets(dir) {
        ...
        const directSkills = immediateDirs
            ...
            .map((entry) => ({ path: entry, name: path.basename(entry), kind: 'skill' }));
        ...
        for (const [file, content] of Object.entries(allContent)) {
            ...
            for (const match of matches) {
                ...
                if (!fragment || fragment.length <= 40)
                    continue;
                if (kind === 'repo' && this.isBenignBase64Fragment(content, match.index ?? 0))
                    continue;
                ...
                if (entry.isDirectory()) {
                    if (kind === 'repo') {
                        if (REPO_EXCLUDE_DIRS.has(entry.name))
                            continue;
                        if (!REPO_SURFACE_DIRS.has(entry.name))
                            continue;
                    }
                    ...
                    if (GENERATED_REPORT_FILES.has(entry.name.toLowerCase()))
                        continue;
                    if (kind === 'repo' && entry.name.startsWith('.'))
                        continue;
                    if (kind === 'repo' && path.dirname(fullPath) === path.resolve(rootDir) && !REPO_ROOT_FILES.has(entry.name))
                        continue;
