...
var import_rust_bridge = require("./rust-bridge.js");
const VERSION = import_capabilities.CAPABILITIES.product.version;
const THRESHOLDS_MAP = {
  normal: { suspicious: 30, malicious: 80 },
  strict: { suspicious: 20, malicious: 60 }
};
const THRESHOLDS = THRESHOLDS_MAP;
...
const DOC_EXTENSIONS = /* @__PURE__ */ new Set([".md", ".txt", ".rst", ".adoc"]);
const DATA_EXTENSIONS = /* @__PURE__ */ new Set([".json", ".yaml", ".yml", ".toml", ".xml", ".csv"]);
...
const GENERATED_REPORT_FILES = /* @__PURE__ */ new Set([
  "guard-scanner-report.json",
  "guard-scanner-report.html",
  "guard-scanner.sarif"
]);
const REPO_MARKERS = /* @__PURE__ */ new Set([
  "package.json",
  "Cargo.toml",
...
const REPO_SURFACE_DIRS = /* @__PURE__ */ new Set([
  "src",
  "dist",
  "scripts",
  "hooks",
...
const REPO_ROOT_FILES = /* @__PURE__ */ new Set([
  "package.json",
  "Cargo.toml",
  "openclaw.plugin.json",
  "pyproject.toml",
...
const REPO_EXCLUDE_DIRS = /* @__PURE__ */ new Set([
  ".git",
  ".jj",
  "node_modules",
  "target",
...
class GuardScanner {
  ...
  scannerDir;
  ignoredSkills = /* @__PURE__ */ new Set();
  ignoredPatterns = /* @__PURE__ */ new Set();
  customRules = [];
  constructor(options = {}) {
  ...
  loadPlugin(pluginPath) {
    try {
      ...
      if (plugin.patterns && Array.isArray(plugin.patterns)) {
        ...
        if (!this.summaryOnly) {
          console.log(`\u{1F50C} Plugin loaded: ${plugin.name || pluginPath} (${plugin.patterns.length} rule(s))`);
        }
      }
    } catch (e) {
  ...
  loadCustomRules(rulesFile) {
    try {
      ...
      if (!this.summaryOnly && this.customRules.length > 0) {
        console.log(`\u{1F4CF} Loaded ${this.customRules.length} custom rule(s) from ${rulesFile}`);
      }
    } catch (e) {
  ...
  loadIgnoreFile(scanDir) {
    ...
    for (const ignorePath of ignorePaths) {
      ...
      if (this.verbose && (this.ignoredSkills.size || this.ignoredPatterns.size)) {
        console.log(`\u{1F4CB} Loaded ignore file: ${this.ignoredSkills.size} skills, ${this.ignoredPatterns.size} patterns`);
      }
      break;
    }
  }
  ...
  scanDirectory(dir) {
    ...
    const targets = this.resolveTargets(dir);
    console.log(`
\u{1F6E1}\uFE0F  guard-scanner v${VERSION}`);
    console.log(`${"\u2550".repeat(54)}`);
    console.log(`\u{1F4C2} Scanning: ${dir}`);
    console.log(`\u{1F4E6} Targets found: ${targets.length} (mode: ${this.scanMode})`);
    if (this.strict) console.log("\u26A1 Strict mode enabled");
    console.log();
    for (const target of targets) {
      const targetPath = path.resolve(target.path);
      if (this.selfExclude && targetPath === this.scannerDir) {
        if (!this.summaryOnly) console.log(`\u23ED\uFE0F  ${target.name} \u2014 SELF (excluded)`);
        continue;
      }
      if (this.ignoredSkills.has(target.name)) {
        if (!this.summaryOnly) console.log(`\u23ED\uFE0F  ${target.name} \u2014 IGNORED`);
        continue;
      }
      this.scanTarget(target.path, target.name, target.kind);
    }
    this.printSummary();
  ...
  scanSkill(skillPath, skillName) {
    this.scanTarget(skillPath, skillName, "skill");
  }
  scanTarget(targetPath, targetName, kind) {
    this.stats.scanned++;
    const findings = [];
    const fileContents = /* @__PURE__ */ new Map();
    if (kind === "skill" && import_ioc_db.KNOWN_MALICIOUS.typosquats.includes(targetName.toLowerCase())) {
      findings.push({
        severity: "CRITICAL",
        id: "KNOWN_TYPOSQUAT",
        cat: "malicious-code",
    ...
    const files = this.getFiles(targetPath, kind);
    for (const file of files) {
      ...
      this.checkIoCs(content, relFile, findings);
      this.checkPatterns(content, relFile, fileType, findings);
      this.checkSignatures(content, file, findings, kind);
      if (this.customRules.length > 0) {
        this.checkPatterns(content, relFile, fileType, findings, this.customRules);
      }
      const baseName = path.basename(relFile).toLowerCase();
      const skipSecret = baseName.endsWith("-lock.json") || baseName === "package-lock.json" || baseName === "yarn.lock" || baseName === "pnpm-lock.yaml" || baseName === "_meta.json" || baseName === ".package-lock.json";
      if (fileType === "code" && !skipSecret) {
        this.checkHardcodedSecrets(content, relFile, findings);
      }
      if ([".js", ".mjs", ".cjs", ".ts"].includes(ext) && content.length < 2e5) {
        this.checkJSDataFlow(content, relFile, findings);
      }
    }
    if (kind === "skill") {
      this.checkStructure(targetPath, targetName, findings);
      this.checkSkillManifest(targetPath, targetName, findings);
      this.checkCompactionPersistence(targetPath, targetName, findings);
    }
    if (this.checkDeps) this.checkDependencies(targetPath, targetName, findings);
    this.checkHiddenFiles(targetPath, targetName, findings);
    this.checkCrossFile(targetPath, targetName, findings, kind);
    this.checkComplexity(targetPath, targetName, findings, kind);
    this.checkConfigImpact(targetPath, targetName, findings, kind);
    const filtered = findings.filter((f) => !this.ignoredPatterns.has(f.id)).flatMap((finding) => {
    ...
    if (kind === "repo" && filtered.length > 0 && filtered.every((finding) => finding.cat === "structural" || finding.fp_suspected)) {
      risk = Math.min(risk, 24);
    }
    const verdict = this.getVerdict(risk);
    this.stats[verdict.stat]++;
    if (!this.summaryOnly) {
      const label = kind === "repo" ? `${targetName} [repo]` : targetName;
      console.log(`${verdict.icon} ${label} \u2014 ${verdict.label} (risk: ${risk})`);
      if (this.verbose && filtered.length > 0) {
        const byCat = {};
        for (const f of filtered) (byCat[f.cat] = byCat[f.cat] || []).push(f);
        for (const [cat, findings2] of Object.entries(byCat)) {
          console.log(`   \u{1F4C1} ${cat}`);
          for (const f of findings2) {
            const icon = f.severity === "CRITICAL" ? "\u{1F480}" : f.severity === "HIGH" ? "\u{1F534}" : f.severity === "MEDIUM" ? "\u{1F7E1}" : "\u26AA";
    ...
    if (filtered.length > 0) {
      const label = kind === "repo" ? `${targetName} [repo]` : targetName;
      this.findings.push({ skill: label, risk, verdict: verdict.label, findings: filtered });
    }
  }
  calibrateFinding(finding, kind, fileContents) {
    ...
    if ((enriched.id === "PI_IGNORE_PREV" || enriched.id === "WORM_REPLICATE") && this.isBenignPromptContext(windowLower)) {
      return null;
    }
  ...
  resolveTargets(dir) {
    ...
    if (this.scanMode === "repo") {
      return [{ path: dir, name: rootName, kind: "repo" }];
    }
    if (this.isSkillDir(dir)) {
      return [{ path: dir, name: rootName, kind: "skill" }];
    }
    const immediateDirs = fs.readdirSync(dir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => path.join(dir, entry.name));
    const directSkills = immediateDirs.filter((entry) => this.isSkillDir(entry)).map((entry) => ({ path: entry, name: path.basename(entry), kind: "skill" }));
    if (directSkills.length > 0) {
      return directSkills;
    }
    const nestedSkills = [];
    for (const parent of immediateDirs) {
      const grandchildren = this.safeReadDirs(parent);
      for (const grandchild of grandchildren) {
        if (this.isSkillDir(grandchild)) {
          nestedSkills.push({
            path: grandchild,
            name: path.basename(grandchild),
            kind: "skill"
          });
        }
      }
    ...
    if (this.scanMode === "skills") {
      return [];
    }
    if (this.looksLikeRepo(dir)) {
      return [{ path: dir, name: rootName, kind: "repo" }];
    }
    return immediateDirs.map((entry) => ({
      path: entry,
  ...
  checkIoCs(content, relFile, findings) {
    ...
    for (const domain of import_ioc_db.KNOWN_MALICIOUS.domains) {
    ...
    for (const fname of import_ioc_db.KNOWN_MALICIOUS.filenames) {
    ...
    for (const user of import_ioc_db.KNOWN_MALICIOUS.usernames) {
  ...
  checkPatterns(content, relFile, fileType, findings, patterns = import_patterns.PATTERNS) {
  ...
  checkSignatures(content, filePath, findings, kind) {
    const contentHash = crypto.createHash("sha256").update(content).digest("hex");
    const relFile = path.basename(filePath);
    for (const sig of import_ioc_db.SIGNATURES_DB.signatures) {
      if (sig.hash && sig.hash === contentHash) {
        findings.push({
          severity: sig.severity,
          id: `SIG_${sig.id}`,
          cat: "signature-match",
          desc: `[${sig.id}] ${sig.name} \u2014 exact hash match`,
          file: relFile
        });
        continue;
      }
      if (sig.patterns) {
        const matchedPatterns = sig.patterns.filter((pat) => content.includes(pat));
        const requiredMatches = kind === "repo" ? Math.min(2, sig.patterns.length) : 1;
        if (matchedPatterns.length < requiredMatches) {
          continue;
        }
        for (const pat of sig.patterns) {
          if (content.includes(pat)) {
            ...
            findings.push({
              ...
              id: `SIG_${sig.id}`,
              cat: "signature-match",
              desc: `[${sig.id}] ${sig.name} (${matchedPatterns.length} indicators)`,
              file: relFile,
              line: lineNum,
        ...
        for (const domain of sig.domains) {
          if (content.includes(domain)) {
            ...
            findings.push({
              ...
              id: `SIG_${sig.id}`,
              cat: "signature-match",
              desc: `[${sig.id}] Suspicious domain: ${domain}`,
              file: relFile,
              line: lineNum
            });
          }
  ...
  checkCompactionPersistence(skillPath, skillName, findings) {
    const files = this.getFiles(skillPath, "skill");
    for (const file of files) {
      const ext = path.extname(file).toLowerCase();
      if (BINARY_EXTENSIONS.has(ext)) continue;
  ...
  checkDependencies(skillPath, skillName, findings) {
    ...
    const RISKY_PACKAGES = /* @__PURE__ */ new Set([
      "node-ipc",
      "colors",
      "faker",
      "event-stream",
  ...
  checkSkillManifest(skillPath, skillName, findings) {
    ...
    if (!fmMatch) return;
    const fm = fmMatch[1];
    const DANGEROUS_BINS = /* @__PURE__ */ new Set([
      "sudo",
      "rm",
      "rmdir",
      "chmod",
  ...
  checkComplexity(skillPath, skillName, findings, kind) {
    const files = this.getFiles(skillPath, kind);
    const MAX_LINES = 1e3;
    const MAX_NESTING = 5;
  ...
  checkConfigImpact(skillPath, skillName, findings, kind) {
    const files = this.getFiles(skillPath, kind);
    for (const file of files) {
      const ext = path.extname(file).toLowerCase();
  ...
  checkJSDataFlow(content, relFile, findings) {
    ...
    const importedModules = /* @__PURE__ */ new Set([...imports.values()]);
  ...
  checkCrossFile(skillPath, skillName, findings, kind) {
    const files = this.getFiles(skillPath, kind);
    const allContent = {};
    for (const file of files) {
      const ext = path.extname(file).toLowerCase();
      ...
      try {
        ...
        if (content.length < 5e5) allContent[relFile] = content;
      } catch {
      }
    }
    ...
    for (const [file, content] of Object.entries(allContent)) {
      const matches = content.matchAll(/[A-Za-z0-9+/]{20,}={0,2}/g);
      for (const match of matches) {
        const fragment = match[0];
        if (!fragment || fragment.length <= 40) continue;
        if (kind === "repo" && this.isBenignBase64Fragment(content, match.index ?? 0)) continue;
        b64Fragments.push({ file, fragment: fragment.substring(0, 30) });
      }
    }
  ...
  getLineText(content, line) {
    if (!content || !line || line < 1) return "";
    return content.split("\n")[line - 1] ?? "";
  }
  getLineWindow(content, line, radius = 2) {
    if (!content) return [];
    const lines = content.split("\n");
    if (!line || line < 1) return lines.slice(0, Math.min(lines.length, radius * 2 + 1));
    const start = Math.max(0, line - 1 - radius);
    const end = Math.min(lines.length, line + radius);
    return lines.slice(start, end);
  }
  isRepoMetadataEmailContext(fileLower, lineLower, windowLower, sample) {
    const manifestFile = fileLower === "cargo.toml" || fileLower === "package.json" || fileLower.endsWith("/package.json");
    if (manifestFile && /(authors|maintainers|email|repository|homepage)/.test(windowLower)) {
      return true;
    }
    const sampleLower = (sample || "").toLowerCase();
    return sampleLower.endsWith("@example.com") || sampleLower.endsWith("@guava.institute") || lineLower.includes("a@example.com");
  }
  isBenignPromptContext(windowLower) {
    return /#\[cfg\(test\)\]|#\[test\]|mod tests|assert!|assert_eq!/.test(windowLower) || /examples:|usage:|help_text|eprintln!\(|println!\(|demo guard/.test(windowLower);
  }
  isPatternCatalogContext(lineLower, windowLower) {
    return /blocked_patterns|network_blocked_patterns|patterns:|regex|desc:|severity|chars|decode_table/.test(windowLower) || /"eval\("|'eval\('|exec\(|spawn\(/.test(lineLower);
  }
  isFirstPartyEvidenceContext(fileLower, lineLower, windowLower, finding) {
    ...
    const isScannerCatalog = (/^(scanner\.js)$/.test(fileLower) || /(src|dist)\/scanner\.js$/.test(fileLower)) && (/dangerous_cfg|compactionpatterns|regex:|label:|desc:|severity:|source of truth|samplelower|example\.com/.test(windowLower) || ["config-impact", "signature-match", "persistence", "system-prompt-leakage", "pii-exposure"].includes(finding.cat));
  ...
  isSchemaFieldContext(lineLower, windowLower) {
    return /\bpub cron\s*:|cron expr|5-field cron|jsonschema|timezone|payload message|task name/.test(windowLower) || /\bpub cron\s*:/.test(lineLower);
  }
  isBenignSecretContext(lineLower, windowLower, sample) {
    const sampleLower = (sample || "").toLowerCase();
    return /abcdefghijklmnopqrstuvwxyz|decode_table|base64|hex|placeholder|0x123456|assert_eq!|#\[test\]|mod tests/.test(windowLower) || /application\/x-www-form-urlencoded|content-type|authorization|format!\("basic \{\}"/.test(windowLower) || sampleLower.startsWith("abcdefgh") || sampleLower.startsWith("0x123456") || sampleLower.startsWith("applicat");
  }
  isBenignBase64Fragment(content, index) {
    const start = Math.max(0, index - 120);
    const end = Math.min(content.length, index + 120);
    const window = content.slice(start, end).toLowerCase();
    return /base64|decode_table|chars|alphabet|#\[test\]|assert_eq!|0x123456|hex/.test(window);
  }
  // ── Risk Scoring ──────────────────────────────────────────────────────
  calculateRisk(findings) {
    ...
    if (findings.some(
      (f) => (f.id === "IOC_IP" || f.id === "IOC_URL" || f.id === "KNOWN_TYPOSQUAT") && !f.fp_suspected
    )) score = 100;
  ...
  scoreFindings(findings) {
    const rust = (0, import_rust_bridge.scoreWithRust)(findings);
    if (rust) {
  ...
  getVerdict(risk) {
    if (risk >= this.thresholds.malicious) return { icon: "\u{1F534}", label: "MALICIOUS", stat: "malicious" };
    if (risk >= this.thresholds.suspicious) return { icon: "\u{1F7E1}", label: "SUSPICIOUS", stat: "suspicious" };
    if (risk > 0) return { icon: "\u{1F7E2}", label: "LOW RISK", stat: "low" };
    return { icon: "\u{1F7E2}", label: "CLEAN", stat: "clean" };
  }
  // ── File Discovery ────────────────────────────────────────────────────
  getFiles(dir, kind, rootDir = dir) {
    const results = [];
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (kind === "repo") {
            ...
            if (!REPO_SURFACE_DIRS.has(entry.name)) continue;
          } else if (entry.name === ".git" || entry.name === ".jj" || entry.name === "node_modules") {
            continue;
          }
          results.push(...this.getFiles(fullPath, kind, rootDir));
        } else {
          if (GENERATED_REPORT_FILES.has(entry.name.toLowerCase())) continue;
          if (kind === "repo" && entry.name.startsWith(".")) continue;
          if (kind === "repo" && path.dirname(fullPath) === path.resolve(rootDir) && !REPO_ROOT_FILES.has(entry.name)) continue;
  ...
  printSummary() {
    ...
    console.log(`
${"\u2550".repeat(54)}`);
    console.log(`\u{1F4CA} guard-scanner v${VERSION} Scan Summary`);
    console.log(`${"\u2500".repeat(54)}`);
    console.log(`   Scanned:      ${total}`);
    console.log(`   \u{1F7E2} Clean:       ${this.stats.clean}`);
    console.log(`   \u{1F7E2} Low Risk:    ${this.stats.low}`);
    console.log(`   \u{1F7E1} Suspicious:  ${this.stats.suspicious}`);
    console.log(`   \u{1F534} Malicious:   ${this.stats.malicious}`);
...
\u26A1 ${this.stats.suspicious} suspicious skill(s) found \u2014 review recommended.`);
  ...
  toJSON() {
    ...
    for (const sr of this.findings) {
      ...
      if (cats.has("prompt-injection")) actions.push("\u{1F6D1} Contains prompt injection patterns.");
      if (cats.has("malicious-code")) actions.push("\u{1F6D1} Contains potentially malicious code.");
      if (cats.has("credential-handling") && cats.has("exfiltration")) actions.push("\u{1F480} CRITICAL: Credential + exfiltration. DO NOT INSTALL.");
      if (cats.has("dependency-chain")) actions.push("\u{1F4E6} Suspicious dependency chain.");
      if (cats.has("obfuscation")) actions.push("\u{1F50D} Code obfuscation detected.");
      if (cats.has("secret-detection")) actions.push("\u{1F511} Possible hardcoded secrets.");
      if (cats.has("memory-poisoning")) actions.push("\u{1F9E0} MEMORY POISONING: Agent memory modification.");
      if (cats.has("prompt-worm")) actions.push("\u{1FAB1} PROMPT WORM: Self-replicating instructions.");
      if (cats.has("data-flow")) actions.push("\u{1F500} Suspicious data flow patterns.");
      if (cats.has("identity-hijack")) actions.push("\u{1F512} IDENTITY HIJACK: Agent soul file tampering.");
      if (cats.has("compaction-persistence")) actions.push("\u23F0 COMPACTION PERSISTENCE: Survives context compaction.");
      if (cats.has("signature-match")) actions.push("\u{1F3AF} SIGNATURE MATCH: Known threat signature detected.");
      if (cats.has("config-impact")) actions.push("\u2699\uFE0F CONFIG IMPACT: Modifies OpenClaw configuration.");
      if (cats.has("pii-exposure")) actions.push("\u{1F194} PII EXPOSURE: Handles personal information.");
    ...
    return {
      ...
      signaturesVersion: import_ioc_db.SIGNATURES_DB.version
    };
  }
  toSARIF(scanDir) {
    const rules = [];
    ...
    for (const sr of this.findings) {
      for (const f of sr.findings) {
        if (!ruleIndex[f.id] && ruleIndex[f.id] !== 0) {
          ...
          const patternDef = import_patterns.PATTERNS.find((p) => p.id === f.id);
    ...
    return {
      ...
      runs: [{
        ...
        results,
        invocations: [{ executionSuccessful: true, endTimeUtc: (/* @__PURE__ */ new Date()).toISOString() }]
      }]
    };
  }
}
