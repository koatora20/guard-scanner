...
function writeJson(outputDir, filename, payload) {
  ...
  (0, import_node_fs.writeFileSync)((0, import_node_path.join)(outputDir, filename), `${JSON.stringify(payload, null, 2)}
`);
}
function buildReports(root) {
  ...
  const boundaryDoc = (0, import_node_fs.readFileSync)((0, import_node_path.join)(root, "docs", "public-safe-boundary.md"), "utf8");
  ...
  const { handleMcpRequest } = require((0, import_node_path.join)(root, "dist", "mcp.js"));
  const npmLatest = JSON.parse(run("npm", ["view", "openclaw", "version", "--json"], root));
  const packDir = (0, import_node_fs.mkdtempSync)((0, import_node_path.join)((0, import_node_os.tmpdir)(), "guard-scanner-pack-report-"));
  const packReport = JSON.parse(run("npm", ["pack", "--json", "--pack-destination", packDir], root));
  const tarballName = packReport[0]?.filename;
  if (tarballName) {
    (0, import_node_fs.rmSync)((0, import_node_path.join)(packDir, tarballName), { force: true });
  }
...
function main() {
  ...
  const outputDir = outputIndex >= 0 ? (0, import_node_path.resolve)(args[outputIndex + 1]) : null;
  ...
  if (outputDir) {
    ...
    (0, import_node_fs.writeFileSync)((0, import_node_path.join)(outputDir, "final-oss-completion-report.md"), `${renderMarkdown(report)}
`);
  }
  console.log(JSON.stringify(report, null, 2));
}
main();
//# sourceMappingURL=oss-release-report.js.map