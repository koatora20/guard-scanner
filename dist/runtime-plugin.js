...
var runtime_plugin_exports = {};
__export(runtime_plugin_exports, {
  buildAuditEntry: () => buildAuditEntry,
  computeRuntimeGuardState: () => computeRuntimeGuardState,
  default: () => runtimeGuardPlugin
});
module.exports = __toCommonJS(runtime_plugin_exports);
...
function resolveConstitutionPath(config) {
  ...
  if ((0, import_node_path.isAbsolute)(config.constitutionPath)) return config.constitutionPath;
  return (0, import_node_path.join)(process.cwd(), config.constitutionPath);
}
function loadConstitutionPolicy(config) {
  const policyPath = resolveConstitutionPath(config);
  if (!policyPath) return null;
  try {
    return JSON.parse((0, import_node_fs.readFileSync)(policyPath, "utf8"));
  } catch {
    return null;
  }
}
...
function normalizeForMatch(value) {
  return (0, import_node_path.normalize)(value).replace(/\\/g, "/");
}
...
function resolveAuditDir(config) {
  return config?.auditDir || (0, import_node_path.join)((0, import_node_os.homedir)(), ".openclaw", "guard-scanner");
}
function resolveAuditFile(config) {
  return (0, import_node_path.join)(resolveAuditDir(config), "audit.jsonl");
}
function ensureAuditDir(config) {
  try {
    (0, import_node_fs.mkdirSync)(resolveAuditDir(config), { recursive: true });
  } catch {
  }
}
function logAudit(entry, config) {
  ...
  try {
    (0, import_node_fs.appendFileSync)(resolveAuditFile(config), line);
  } catch {
  }
}
function resolveSuiteTokenPath(config) {
  return config?.suiteTokenPath || (0, import_node_path.join)((0, import_node_os.homedir)(), ".openclaw", "guava-suite", "token.jwt");
}
function resolveConfigPath(config) {
  return config?.configPath || (0, import_node_path.join)((0, import_node_os.homedir)(), ".openclaw", "openclaw.json");
}
function isSuiteActive(config) {
  try {
    const token = (0, import_node_fs.readFileSync)(resolveSuiteTokenPath(config), "utf8").trim();
...
function loadMode(config) {
  ...
  try {
    const runtimeConfig = JSON.parse((0, import_node_fs.readFileSync)(resolveConfigPath(config), "utf8"));
