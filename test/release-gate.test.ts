...
test("release gate validates lean packed artifact", () => {
    const output = execFileSync("node", ["dist/tools/release-gate.js"], {
        cwd: process.cwd(),
        encoding: "utf8",
    });
    ...
    assert.equal(report.packedFiles.includes("dist/index.mjs"), true);
    assert.equal(report.packedFiles.includes("dist/index.d.ts"), true);
    assert.equal(report.packedFiles.includes("dist/plugin.js"), true);
    assert.equal(report.packedFiles.includes("dist/mcp.js"), true);
    assert.equal(report.packedFiles.includes("dist/public-types.d.ts"), true);
