...
プロンプトインジェクション、アイデンティティ乗っ取り、メモリ汚染など23以上の脅威を検出。
...
[![Tests Passing](https://img.shields.io/badge/tests-136%2F136-brightgreen?style=flat-square)]()
[![OWASP Agentic](https://img.shields.io/badge/OWASP_Agentic-90%25-green?style=flat-square)]()
[![Patterns](https://img.shields.io/badge/patterns-147-blueviolet?style=flat-square)]()
...
| 機能 | 説明 |
|------|------|
| **23脅威カテゴリ** | Snyk ToxicSkills + OWASP Agentic Top 10 + アイデンティティ乗っ取り + PII + 信頼悪用 + VDB Injection |
| **147静的パターン** | 正規表現ベースの静的解析（コード、ドキュメント、データファイル対応） |
...
guard-scannerは4つのソースに基づく**23の脅威カテゴリ**をカバー:
...
| 15 | **CVEパターン** | CRITICAL | CVE-2026-25253 (RCE)、CVE-2026-25905 (Pyodide)、CVE-2026-27825 (path traversal) |
...
| 23 | **VDB Injection** | HIGH | ベクトルDB汚染、エンベディング操作 |

> **カテゴリ17〜23** はguard-scanner独自の検出項目です。カテゴリ17（アイデンティティ乗っ取り）は実際の攻撃から開発されました。
...
| パターンDB | 4 | 147パターン、必須フィールド、23カテゴリカバレッジ |
...
| OWASP Agentic Security | 14 | ASI01–10全テスト |
| ランタイムガード | 25 | 5層全チェック・3モード・CVE検出 |
